...
if __name__ == "__main__":
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for batch in tqdm(train_loader, desc=f"[Epoch {epoch}] Training", leave=False):
            ...
            with autocast("cuda"):
                if model_name == "CNNMoE" or model_name == "FastMoE":
    ...
    df.to_csv(f"results/{model_name}_{num_epochs}_epochs_single_run_results.csv", index=False)
    print(f"Results saved to results/{model_name}_{num_epochs}_epochs_single_run_results.csv (time elapsed: {time_elapsed:.2f}s)")

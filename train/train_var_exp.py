...
num_experts = [1,2,4,8]  # Fewer experts to test
num_epochs = 2
num_obs = 1000
cuda = 0
...
if __name__ == "__main__":
    ...
    for model_class, model_name in zip(model_classes, model_names):
        ...
        for num_expert in num_experts:
            ...
            eval_loader = DataLoader(dataset, batch_size=16, shuffle=False, num_workers=0, pin_memory=False)
            ...
            for epoch in range(1, num_epochs + 1):
                ...
                for batch in tqdm(train_loader, desc=f"[Epoch {epoch}] Training", leave=False, disable=True):
                ...
                with torch.no_grad(), autocast("cuda"):
                    for batch in tqdm(val_loader, desc=f"[Epoch {epoch}] Validation", leave=False, disable=True):
                        x, y, bld_mask = prepare_batch(batch, device)
                        preds, aux_loss = model(x)
                        main_loss = masked_mse_loss(preds, y, bld_mask)

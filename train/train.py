...
# Config
model_classes = [CNNMoE, TransUNet, UNet]
model_names = ["CNNMoE", "TransUNet", "UNet"]  # For saving files
num_epochs = 2
...
if __name__ == "__main__":
    ...
    for model_class, model_name in zip(model_classes, model_names):
        ...
        for num_obs in num_obser:
            ...
            df.to_csv(f"results/{model_name}_{num_epochs}_epochs_results.csv", index=False)
            print(f"Results saved to results/{model_name}_{num_epochs}_epochs_results.csv (time elapsed: {time_elapsed:.2f}s)")
